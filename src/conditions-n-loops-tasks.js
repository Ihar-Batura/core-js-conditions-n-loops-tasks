...
function convertToRomanNumerals(num) {
  ...
  for (let i = 0; i < romanMatrix.length; i += 1) {
    while (newNum >= romanMatrix[i][0]) {
      res += romanMatrix[i][1];
      newNum -= romanMatrix[i][0];
    }
  }
  return res;
}
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    ...
    for (let j = i + 2; j < arr.length; j += 1) {
      sumRight += arr[j];
      if (sumLeft === sumRight && j + 1 === arr.length) {
        return arrIndex;
      }
    }
    sumRight = 0;
...
function rotateMatrix(matrix) {
  ...
  for (let i = n - 1; i > n / 2 - 1; i -= 1) {
    for (let j = i; j > n - i - 1; j -= 1) {
      const temp = matrix[i][j];
      mat[i][j] = matrix[n - j - 1][i];
      mat[n - j - 1][i] = matrix[n - i - 1][n - j - 1];
      mat[n - i - 1][n - j - 1] = matrix[j][n - i - 1];
      mat[j][n - i - 1] = temp;
    }
  }
  return matrix;
